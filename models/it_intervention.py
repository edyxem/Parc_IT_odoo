...
class ItIntervention(models.Model):
    ...
    def action_export_couts_excel(self):
        ...
        for row, inv in enumerate(interventions, start=2):
            ...
            ws.write(row, 2, inv.equipement_id.name, fmt_cell)
            ws.write(row, 3, inv.type_intervention, fmt_cell_center)
            ws.write(
                row, 4,
                inv.technicien_id.name if inv.technicien_id else '—',
                fmt_cell
            )
            if inv.date_debut:
                ws.write(
        ...
        data_mois = defaultdict(lambda: defaultdict(float))
        mois_set = set()
        eq_set = set()
        ...
        for col, mois in enumerate(mois_list, start=1):
            mois_fmt = datetime.strptime(
                mois, '%Y-%m').strftime('%b %Y')
            ws2.write(1, col, mois_fmt, fmt_mois_header)
            ws2.set_column(col, col, 14)
        ws2.write(1, len(mois_list) + 1, 'TOTAL', fmt_header)
        ws2.set_row(1, 28)
        ...
        for row, eq_name in enumerate(eq_list, start=2):
            ...
            ws2.write(row, len(mois_list) + 1, total_eq, fmt_total)
